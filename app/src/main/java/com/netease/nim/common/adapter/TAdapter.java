package com.netease.nim.common.adapter;
...
public class TAdapter<T> extends BaseAdapter implements IViewReclaimer {
	...
	public TAdapter(Context context, List<T> items, TAdapterDelegate delegate, SessionCustomization customization) {
	...
	public View getView(final int position, View convertView, ViewGroup parent, boolean needRefresh) {
		...
		//是否显示patient提示
		if(customization!=null&&customization.type== ConstantValues.P2P_REPORT){
			holder.setShowPatientTip(false);
		}else{
			holder.setShowPatientTip(true);
		}
		...
		if (needRefresh) {
            try {
                ...
                LogUtils.e("TAdapter", "refresh viewholder error. " + e.getMessage());
            }
		}
	...
	public View viewAtPosition(int position) {
		...
		//是否显示patient提示
		if(customization!=null&&customization.type== ConstantValues.P2P_REPORT){
			holder.showPatientTip=false;
		}else{
			holder.showPatientTip=true;
		}
