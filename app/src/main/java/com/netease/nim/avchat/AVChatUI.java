...
import com.netease.nim.avchat.activity.AVChatExitCode;
import com.netease.nim.avchat.constant.CallStateEnum;
...
public class AVChatUI implements AVChatUIListener {
    ...
    public AVChatUI(Context context, View root, AVChatListener listener, String order, NetCallback callback, Handler task) {
     ...
     * 拨打音视频
     */
    public void outGoingCalling(String account, AVChatType callTypeEnum, String order) {
        ...
        notifyOption.extendMessage = order;
        ...
        AVChatManager.getInstance().call(account, callTypeEnum, videoParam, notifyOption, new AVChatCallback<AVChatData>() {
            ...
            public void onFailed(int code) {
                LogUtils.d("avChat call failed code->" + code);
                DialogMaker.dismissProgressDialog();
            ...
            public void onException(Throwable exception) {
                LogUtils.d("avChat call onException->" + exception);
                DialogMaker.dismissProgressDialog();
            }
        });
    }
    ...
    public void hangUp(final int type) {
        if (type == AVChatExitCode.HANGUP || type == AVChatExitCode.PEER_NO_RESPONSE || type == AVChatExitCode.CANCEL) {
            AVChatManager.getInstance().hangUp(new AVChatCallback<Void>() {
                ...
                @Override
                public void onFailed(int code) {
                    LogUtils.d("hangup onFailed->" + code);
                }

                @Override
                public void onException(Throwable exception) {
                    LogUtils.d("hangup onException->" + exception);
                }
            });
        }
        closeSessions(type);
        ...
        switch (code) {
            ...
            case AVChatExitCode.HANGUP:
                if (isCallEstablish.get()) {
                    ...
                    //zhang  挂断成功
                    BaseManager.textOrdeTypeUpdate(order, 4 + "", mCallback);
                }
                break;
    ...
    private void rejectInComingCall() {
        ...
        AVChatManager.getInstance().hangUp(new AVChatCallback<Void>() {
            ...
            @Override
            public void onFailed(int code) {
                LogUtils.d("reject sucess->" + code);
            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.d("reject sucess");
            }
        });
    ...
    private void receiveInComingCall() {
        ...
        AVChatManager.getInstance().accept(videoParam, new AVChatCallback<Void>() {
            ...
            public void onSuccess(Void v) {
                ...
                LogUtils.i("accept success");
                long time = 15 * 60 * 1000;
                long time1 = 1 * 30 * 1000;//1s 为1000ms
                mHandler.sendEmptyMessageDelayed(0, time1);
            }
            ...
            public void onFailed(int code) {
                ...
                LogUtils.e("accept onFailed->" + code);
                closeSessions(AVChatExitCode.CANCEL);
            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.d("accept exception->" + exception);
            }
        });
    }
    ...
    public void audioSwitchVideo() {
        ...
        AVChatManager.getInstance().requestSwitchToVideo(videoParam, new AVChatCallback<Void>() {
            @Override
            public void onSuccess(Void aVoid) {
                LogUtils.d("requestSwitchToVideo onSuccess");
            }

            @Override
            public void onFailed(int code) {
                LogUtils.d("requestSwitchToVideo onFailed" + code);
            }

            @Override
            public void onException(Throwable exception) {
                LogUtils.d("requestSwitchToVideo onException" + exception);
            }
        });
    }
