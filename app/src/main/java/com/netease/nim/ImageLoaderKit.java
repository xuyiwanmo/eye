...
import com.netease.nim.common.media.picker.util.BitmapUtil;
...
public class ImageLoaderKit {
    ...
    private void init(ImageLoaderConfiguration config) {
        try {
            ...
            LogUtils.e(TAG, "init ImageLoaderKit error, e=" + e.getMessage().toString());
        }

        LogUtils.i(TAG, "init ImageLoaderKit completed");
    }
    ...
    private ImageLoaderConfiguration getDefaultConfig() throws IOException {
        ...
        LogUtils.i(TAG, "ImageLoader memory cache size = " + MAX_CACHE_MEMORY_SIZE / M + "M");
        LogUtils.i(TAG, "ImageLoader disk cache directory = " + cacheDir.getAbsolutePath());
    ...
    public static void buildAvatarCache(List<String> accounts) {
        ...
        LogUtils.i(TAG, "build avatar cache completed, vatar count =" + accounts.size());
    }
