 ...
 */
public class CommentAdapter extends CommAdapter<Comment> {
    public CommentAdapter(Context context, ArrayList<Comment> list, int layoutId) {
    ...
    protected View convert(Comment comment, View convertView, Context context, int position, int layoutId) {
        ...
        if(position==0){
            viewHolder.mMessageImg.setVisibility(View.VISIBLE);
        }else{
            viewHolder.mMessageImg.setVisibility(View.INVISIBLE);
        }
